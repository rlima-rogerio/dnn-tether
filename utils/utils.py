...
def read_dataset(root_dir, archive_name, dataset_name):
    ...
    if archive_name == 'mts_archive':
        ...
        # print('+++')
        # # print(x_test.mean(axis=1, keepdims=True))
        # print(std_)
        # print('+++')

        datasets_dict[dataset_name] = (x_train.copy(), y_train.copy(), x_test.copy(),
                                       y_test.copy())
    elif archive_name == 'UCRArchive_2018_Continuous':
        root_dir_dataset = cur_root_dir + '/archives/' + 'UCRArchive_2018' + '/' + dataset_name + '/' + 'Continuous'
        df_train = pd.read_csv(root_dir_dataset + '/' + dataset_name + 'Y_TEST.tsv', sep='\t', header=None)

        df_test = pd.read_csv(root_dir_dataset + '/' + dataset_name + 'X_TEST.tsv', sep='\t', header=None)

        # y_train = df_train.values[:, 0]
        # y_test = df_test.values[0, :]
        y_test = df_train.values[0, :]
        ...
        # print('mean = %f.3, std = %f.3' % (x_test.mean(axis=1, keepdims=True), std_))
        # print('+++')
        # print(x_test.mean(axis=1, keepdims=True))
        # print('+++')

        datasets_dict[dataset_name] = (x_test.copy(), y_test.copy())
    else:
...
def transform_mts_to_ucr_format():
    ...
    mts_root_dir = '/home/lima/Documents/classes/ann/dl-4-tsc/archives/mtsdata/'
    mts_out_dir = '/home/lima/Documents/classes/ann/dl-4-tsc/archives/mts_archive/'
    for dataset_name in MTS_DATASET_NAMES:
...
def single_evaluation(root_dir, classifier):
    ...
    # Workaround to overcome the limitation to perform single predictions. The input array (x_test)
    # is repeated to build a matrix of dimension (N=82,d=300) 
    #   where N is the number of input tests and 
    #         d is the number of samples.
    # Ideally, the prediction should be performed over the array (1 x 300).

    # single_test = x_test[idx]
    # single_test = single_test.reshape(1,300)

    # DEBUG
    # print(y_test.size)
    # print(x_test.size)
    for x in range(y_test.size):
        single_test = np.tile(x_test[x],(y_test.size,1))

        # st = np.tile(x_test[x,0:],(y_test.size,1))
        # print(st[0,0:4])

        print(single_test[0,0:4])
        # print('---')

        y_pred = model.predict(single_test)
        print('i = %d, Expected = %d, Predicted = %d' % (x, y_test[x].astype(numpy.int16), 
                                                         y_pred[x].argmax()))



    # y_pred = model.predict(x_test[idx],batch_size=1) # Remove after debugging
    # y_pred = model.predict(np.array(x_test[idx]))[0] #np.array( [single_x_test,] 
    # y_pred = model.predict(single_test)

    # print(y_pred[idx])
    # print(y_pred.size)
    # print(y_pred)
    # print(y_test[idx:2*idx])
    # print([y_test, y_pred[idx].argmax(axis=1)])
    # print(y_test)
    # print(y_pred.argmax(axis=1))
    # print(numpy.vstack( (y_test, y_pred.argmax(axis=1)) ))
    # print('---')
    # print(x_test[idx].size)
    # print(x_test[idx])

    # Keep this
    # print(y_pred[idx].argmax())
...
def batch_evaluation(root_dir, classifier, data_portion):
    ...
    if (data_portion == 'train'):
        x = datasets_dict[dataset_name][0]
        y = datasets_dict[dataset_name][1]
    elif (data_portion == 'test'):
        x = datasets_dict[dataset_name][2]
        y = datasets_dict[dataset_name][3]
    ...
    matrix = confusion_matrix(y, y_pred.argmax(axis=1),normalize='all')
    print(matrix)

    plt.rcParams.update({'font.size': 12})
    cmd_obj = ConfusionMatrixDisplay(matrix, display_labels=['No Collision', 'Soft', 'Hard'])
    fig, ax = plt.subplots(figsize = (10,8))
    cmd_obj.plot(ax = ax)
    cmd_obj.ax_.set(
                title=classifier.capitalize(), 
                xlabel='Predicted States', 
                ylabel='Actual States')

    label_font = {'size':'16'}  # Adjust to fit
    ax.set_xlabel('Predicted States', fontdict=label_font)
    ax.set_ylabel('Actual States', fontdict=label_font)
    ...
    # y_pred_bin = label_binarize(y_pred.argmax(axis=1), classes=[0, 1, 2])
...
def continuous_evaluation(root_dir, classifier):
    import tensorflow.keras as keras
    ...
    classifier = 'inception'
    archive_name = 'UCRArchive_2018'
    archive_name_continuous = 'UCRArchive_2018_Continuous'
    dataset_name = 'Collision'
    datasets_dict = read_dataset(root_dir, archive_name_continuous, dataset_name)

    x_test = datasets_dict[dataset_name][0]
    y_test = datasets_dict[dataset_name][1]
    ...
    # xt = tf.keras.Input(shape=(300,1), dtype=tf.float32)
    # print(xt.shape)
    ...
    loopsize = 3000
    # y_pred_array = np.zeros(loopsize) #x_test.size - window_size
    # xv = np.zeros(loopsize) 

    window_size = 300
    y_pred_array = np.zeros(x_test.size - window_size) #x_test.size - window_size
    xv = np.zeros(x_test.size - window_size)  

    # for x in range(loopsize):
    for x in range(x_test.size - window_size):

        window = x_test[0,x:x + window_size]
        std_ = window.std()
        window = (window - window.mean()) / std_
        
        single_test = np.tile(window,(82,1))
        
        y_pred = model.predict(single_test)
        print('i = %d, Expected = %d, Predicted = %d' % (x, y_test[x].astype(numpy.int16), 
                                                         y_pred[0].argmax()))    # take the 1st value (all remaining ones are equal)
        y_pred_array[x] = y_pred[0].argmax()
        xv[x] = x
        
        f.write('%d, %d, %d \n' %(x, y_test[x].astype(numpy.int16), y_pred[0].argmax())) 

        # print('======')
        # xt = x_test[0,0:300]
        # xt = np.reshape([x_test[0,0:300]], (1, 300, 1))
        # xt = np.resize([x_test[0,0:300]], (300, 1))
        # xt = np.expand_dims(xt, axis=0)
        # xt = x_test[0,0:300]
        # print(xt.shape)
        # print(x_test[0,0:300].size)
        # y_pred = model.predict(x_test[0,x:x + window_size])
        # y_pred = model.predict(xt)
        # Keep this
        # print(y_pred.argmax())
        # print(y_pred)
        # print(y_test[x].astype(numpy.int16))    
        # print('------')
        # print('+---------------+')
        # print(y_test[x])
        # print('+---------------+')
    ...
    # plot
    fig, ax = plt.subplots()
    ax.step(xv, y_pred_array, linewidth=2.0, label='Continuous')
    ax.step(xv, y_test[0:xv.size], linewidth=2.0, label='Window')
